...
// Request contains parameters for activating a product.
type Request struct {
	ProductID string
}

// Interactor handles the activate product usecase.
...
func (it *Interactor) Execute(ctx context.Context, req Request) (err error) {
	ctx, end := observability.StartSpan(ctx, "ActivateProduct")
	defer func() { end(err) }()
